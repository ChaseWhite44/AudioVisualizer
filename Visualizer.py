...
class AudioAnalyzer:
    ...
    def load(self, filename):
        time_series, sample_rate = librosa.load(filename)  # librosa reads the song

        # returns 2D-array of frequencies and times
        stft = np.abs(librosa.stft(time_series, hop_length=512, n_fft=2048 * 4))
        self.spectrogram = librosa.amplitude_to_db(stft, ref=np.max)  # changes array to decibel

        # Separates array into 2, frequencies and time
        frequencies = librosa.core.fft_frequencies(n_fft=2048 * 4)
        times = librosa.core.frames_to_time(np.arange(self.spectrogram.shape[1]), sr=sample_rate,
...
class AudioBar:
    ...
    def __init__(self, x, y, freq, color, width=50, min_height=10, max_height=80, min_decibel=-80, max_decibel=0):
...
class AverageAudioBar(AudioBar):
    ...
    def __init__(self, x, y, rng, color, width=50, min_height=30, max_height=80, min_decibel=-80, max_decibel=0):
...
class RotatedAverageAudioBar(AverageAudioBar):
    ...
    def __init__(self, x, y, rng, color, angle=0, width=50, min_height=10, max_height=80, min_decibel=-80,
                 max_decibel=0):
        super().__init__(x, y, 0, color, width, min_height, max_height, min_decibel, max_decibel)
        self.rng = rng
        self.rect = None
