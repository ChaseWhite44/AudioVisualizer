...
# Creates window for visualizer
screen_w = int(infoObject.current_w / 1.3)
screen_h = int(infoObject.current_h / 1.3)
...
circle_color = (40, 40, 40)  # Gray
polygon_default_color = [255, 255, 255]  # White
polygon_bass_color = polygon_default_color.copy()
polygon_color_vel = [0, 0, 0]  # Black
    ...
    for c in g:
        gr.append(
            RotatedAverageAudioBar(circleX + radius * math.cos(math.radians(ang - 90)),
                                   ...
                                   c, (255, 0, 255), angle=ang, width=8, max_height=320))
        ang += angle_dt
...
while running:
    ...
    for b1 in bars:
        for b in b1:
            b.x, b.y = circleX + radius * math.cos(math.radians(b.angle - 90)), \
                       circleY + radius * math.sin(math.radians(b.angle - 90))
            b.update_rect()
